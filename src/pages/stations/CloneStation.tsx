...
export default class CloneStation extends React.Component<IProps, IState> {
        ...
        this.context.instance.acquireTokenSilent({
            ...
            .then((response: any) => {
                apiClient.createStation({
                    Authorization: `Bearer ${response.idToken}`,
                    ApiKey: "",
                    stationcode: this.state.station.station_code,
                    body: this.state.station
                }).then((response: any) => {
                    if (response.right.status === 201) {
                        toast.info("Clonazione avvenuta con successo.");
                        this.setState({station: response.right.value});
                        this.setState({stationName: response.right.value.station_code});
                        this.updateBackup("station", response.right.value);
                    } else {
