...
export default class CdsServices extends React.Component<IProps, IState> {
                ...
                apiClient.getCdsServices({
                    ...
                    .then((res: any) => {
                        if (res.right.status === 200) {
                            this.setState({services: res.right.value});
                        } else {
                ...
                request
                    .then((res: any) => {
                        if (res.right && (res.right.status === 200 || res.right.status === 201)) {
                            ...
                            this.toastError(res.right?.value?.detail ?? "Operazione non avvenuta a causa di un errore");
                        }
                    })
                    .catch(() => {
    ...
    render(): React.ReactNode {
        const {services, isLoading, showFormModal, isEditing, formData, showDeleteModal, serviceToDelete} = this.state;
                ...
                {isLoading && (<FaSpinner className="spinner"/>)}
                {
                    !isLoading && (
                        <Table hover responsive size="sm">
                            ...
                            <tbody>
                            {
                                services.map((service: CdsServizio, index: number) => (
